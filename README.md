...
- **Lançamentos**: entradas, despesas, transferências, com suporte a recorrência (fixa mensal ou parcelada), despesas de cartão de crédito, **divisão de um lançamento em múltiplas categorias** com valores individuais, sugestão automática de categoria com base no histórico de nomes, e filtro rápido por tipo (KPIs clicáveis).
- **Contas e Cartões de Crédito**: controle de saldo, faturas por competência, pagamento de fatura, limite, cartão preferencial, arquivamento.
- **Categorias**: hierarquia categoria/subcategoria, com ícones e cores personalizáveis, opção de ocultar uma categoria dos gráficos do Dashboard (sem afetar os totais), e opção de arquivar ou excluir (com aviso de impacto no histórico).
- **Orçamento**: planejamento mensal por categoria, com vigência recorrente ou pontual, comparado ao realizado, histórico de 6 meses com linha de meta, e indicador circular de utilização no Dashboard.
...
- **Tags**: marcação livre de lançamentos, com contagem de uso, edição inline e remoção rápida.
...
- **Modo escuro** com cobertura completa de contraste (formulários, seletores, ícones nativos de data, gráficos e badges), e paleta de identidade visual própria (navy/teal).
